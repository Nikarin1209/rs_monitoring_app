...
class CompositeIndexScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avgFatigue = AnalyticsService.mean(
      AnalyticsService.fatigueValues(entries30),
    );
    final avgPain = AnalyticsService.mean(
      AnalyticsService.painValues(entries30),
    );
    final avgMood = AnalyticsService.mean(
      AnalyticsService.moodValues(entries30),
    );
    final avgNumbness = AnalyticsService.mean(
      AnalyticsService.numbnessValues(entries30),
    );
    final avgCoordination = AnalyticsService.mean(
      AnalyticsService.coordinationValues(entries30),
    );
    final avgVision = AnalyticsService.mean(
      AnalyticsService.visionValues(entries30),
    );
    final avgWeakness = AnalyticsService.mean(
      AnalyticsService.weaknessValues(entries30),
    );
    final avgStress = AnalyticsService.mean(
      AnalyticsService.stressValues(entries30),
    );

    final int? indexValue =
        (avgFatigue != null && avgPain != null && avgMood != null)
    ...
    final last7 = AnalyticsService.entriesForLastDays(
      diary.diaryEntriesSorted,
      7,
    );
    final prev7 = AnalyticsService.entriesForPreviousDays(
      diary.diaryEntriesSorted,
      7,
    );
    ...
    final weeklyDelta = AnalyticsService.percentChange(last7Avg, prev7Avg);

    // Chart data: per-entry composite index / 10 to fit 0–10 NLChart scale.
    final chartData = entries30
        .map(
          (e) =>
              (AnalyticsService.calculateCompositeIndexFromEntry(e) ?? 0) /
              10.0,
        )
        .toList();
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Text(
              'вес $weight · $displayValue/$maxLabel',
